...
class University extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
			'users' => array(self::MANY_MANY, 'User', 'user_university(university_id, user_id)'),
		);
	}
	...
	{
		if($userId==NULL)
            $universities=$this->findAll();
        else
            $universities=User::model()->findByPk($userId)->universities;
		$names = array();
		foreach($universities as $uni)
		{
			$names[$uni->university_id]=$uni->name;
		}
		return $names;
	}
