...
class ImageExt {
	...
	{
		return new ImageExt($image, $config);
	}
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ( ! isset(ImageExt::$allowed_types[$image_info[2]]))
		...
		(
			...
			'height' => $image_info[1],
			'type' => $image_info[2],
			'ext' => ImageExt::$allowed_types[$image_info[2]],
	...
	protected function valid_size($type, & $value)
	{
		...
		switch ($type)
		{
			...
			break;
			case 'master':
				if ($value !== ImageExt::NONE AND
				    $value !== ImageExt::AUTO AND
				    $value !== ImageExt::WIDTH AND
				    $value !== ImageExt::HEIGHT)
					return FALSE;
			break;
		}
